...
class GraphAttention(Layer):
    ...
    def build(self, input_shapes ):
        ...
        self.delta = self.add_weight(
            name="{}_ig_delta".format(self.name), shape=(), trainable=False, initializer=initializers.ones()
        )
        self.non_exist_edge = self.add_weight(
            name="{}_ig_non_exist_edge".format(self.name),
            shape=(),
            trainable=False,
        ...
        for head in range(self.attn_heads):
            ...
            if self.use_bias:
                bias = self.add_weight(
                    ...
                    name="{}_bias_{}".format(self.name, head),
                )
    ...
    def map_call(self, inputs, A, N, out_indices):
        ...
        for head in range(self.attn_heads):
            ...
            attn_for_self = tf.matmul(
                features, tf.transpose(attention_kernel[0])
            )  # (N x 1), [a_1]^T [Wh_i]
            ...
            # mask = tf.convert_to_tensor(A, dtype=tf.bool)

            dense = A * dense
            # dense_test = dense[~A] = 0
            # print(dense_test[0][0][0:40])

            dense = LeakyReLU(alpha=0.2)(dense)

            dense = tf.where(dense!=0, dense, tf.float32.min)

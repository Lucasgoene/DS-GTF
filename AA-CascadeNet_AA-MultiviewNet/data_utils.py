...
def multi_processing_multiview(directory, length, num_cpu,depth):
    ...
    #results = load_overlapped_data_multiview((directory, depth))
    ...
    for i in range(window_size):
        ...
        x_temp = np.concatenate((x_temp,results[0]["input"+str(i+1)]))
        x_lstm = np.concatenate((x_lstm,results[0]["input"+str(i+window_size+1)]))
...
def load_overlapped_data_multieviewGAT(file_dirs, depth, overlap):
    ...
    # data = np.average(np.swapaxes(rest_matrix, 0, 1), axis=0)
    ...
    # data = np.average(np.swapaxes(math_matrix, 0, 1), axis=0)
    # outliers = abs(data - np.mean(data)) < 3 * np.std(data)
    # outlier_channels = np.where(outliers == False)[0]
    # for channel in outlier_channels:
    ...
    # data = np.average(np.swapaxes(memory_matrix, 0, 1), axis=0)
    # outliers = abs(data - np.mean(data)) < 3 * np.std(data)
    # outlier_channels = np.where(outliers == False)[0]
    # for channel in outlier_channels:
    ...
    # data = np.average(np.swapaxes(motor_matrix, 0, 1), axis=0)
    # outliers = abs(data - np.mean(data)) < 3 * np.std(data)
    # outlier_channels = np.where(outliers == False)[0]
    # for channel in outlier_channels:

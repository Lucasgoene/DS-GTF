...
from tensorflow.keras.layers import Layer, Dropout, LeakyReLU, Linear, Parameter
import tensorflow as tf

def add_self_loops_v2(edge_index, edge_weight: Optional[tf.Tensor] = None,
                      edge_attr: Optional[tf.Tensor] = None, edge_attr_reduce: str = "mean",
    ...
    loop_index = torch.arange(0, N, dtype=tf.long, device=edge_index.device)
    loop_index = loop_index.unsqueeze(0).repeat(2, 1)

    if edge_weight is not None:
        assert edge_weight.numel() == edge_index.size(1)
        loop_weight = edge_weight.new_full((N,), fill_value)
        edge_weight = tf.cat([edge_weight, loop_weight], dim=0)
    ...
    if edge_attr is not None:
        ...
        edge_attr = tf.cat([edge_attr, loop_attr], dim=0)

    edge_index = tf.cat([edge_index, loop_index], dim=1)

    return edge_index, edge_weight, edge_attr


class GATEdgeConv(Layer):
    def __init__(self, in_channels: Union[int, Tuple[int, int]],
        ...
        if self.edge_dim is not None:
            ...
            self.att_e = tf.Variable(tf.Tensor(1, heads, out_channels))
        else:
            self.lin_e = None

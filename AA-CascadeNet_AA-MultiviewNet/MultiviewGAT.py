...
class MultiviewGAT:
    ...
    def get_edge_index(self, gamma):
      ...
      # fake_array = tf.range(1,249)
      # fake_array = tf.expand_dims(fake_array, axis=0)
      # locations = array_to_mesh(fake_array)

      # locations_new = [[0,0]]*248
      # (len_x, len_y) = locations.shape
      # for x in range(0,len_x):
      #    for y in range(0,len_y):
      #       if(locations[x][y] > 0):
      #         locations_new[int(locations[x][y]) - 1] = [x,y]

      # distances_matrix = distance_matrix(locations_new, locations_new)
      # delta = 5
      # adjacency_matrix = tf.minimum(tf.ones([248,248]), tf.math.divide_no_nan((tf.ones([248,248]) * delta),(distances_matrix**2)))  # Computes the adjacency matrix cells as min(1, delta/d_ij), N.B. zero division error can arise here for the diagonal cells, "1" value will be chosen automatically instead
      
      #Add self_loops
      # diag = tf.linalg.diag(tf.ones(248))
    ...
    def graphAttention(self, x):
      ...
      # graph_out = tf.reshape(graph_out, [-1, 20, 21, ])
      return graph_out
